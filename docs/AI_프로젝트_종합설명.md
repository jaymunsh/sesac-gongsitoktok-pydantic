...
| **provenance** (근거추적) | 답의 각 주장이 **어느 원문에서 왔는지 되짚기**. 환각 억제·감사 → 금융권 필수. |
...
### 8-1. 현재 구현 — 실시간 RAG 요약
"사업 내용 요약" → router=summary → corpus에서 **본문(text) 청크 우선 검색**(`summary_top_k=8`,
kind=text 필터) → summary_agent가 그 근거로 **그 자리에서** 서술 요약.
- 장점: 단순·항상 최신. 단점: 매 턴 재검색+재요약(비용·지연), 품질이 그때 검색에 좌우.

### 8-2. 질문 종류별 분기 (시나리오)
- **개요/사업 요약** → 벡터(의미) + 본문 청크 위주(summary 트랙).
- **정확 수치**("매출 얼마?") → **재무 결합 노드(DART 정형 API)** 가 1급, RAG는 백업.
- **혼합**("매출과 환율 영향은?") → qa 트랙 + 재무·거시 병렬 결합.

### 8-3. 원안(미구현) — 사전요약 트랙
아키텍처 원안은 **빌드 시점에** 각 공시 섹션을 싼 모델로 미리 요약해 별도 컬렉션
`summary_<corp>`에 저장하고, 질의 시엔 **완성된 요약을 꺼내쓰기**만 하는 것이다(요약 품질↑·질의
비용↓). 본 재구축에선 아직 실시간 RAG로 처리 — **남은 핵심 개선점**(§15).
...
| 총 청크 | **67,023** (삼성 23,444 + 현대 43,579), cosine 컬렉션 |
...
날짜필터. **사전요약 트랙은 기존엔 있었으나 재구축은 미구현**(개선점). → 재구축의 진짜 가치는
...
| 개선 | 내용 | 가치 |
|---|---|---|
| **사전요약 트랙** | `summary_<corp>` 빌드타임 사전요약 → 요약 품질↑·질의 비용↓ | 아키텍처 원안 완성 |
