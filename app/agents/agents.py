...
router_agent = Agent(
    ...
    instructions=(
        ...
        "- intent: smalltalk(인사·잡담) / qa(특정 사실·수치) / summary(개요·서술) / out_of_scope.\n"
        "- out_of_scope=true 인 경우: ① 방 회사가 아닌 다른 회사가 주제이거나 "
        "② 방 회사와 **다른 회사를 비교**하거나 다른 회사가 함께 언급될 때(예: '삼성전자랑 SK하이닉스 중 누가…'). "
        "이때 detected_company에 그 다른 회사명을 적는다. "
        "회사명이 없으면 방 회사 질문으로 간주(in scope). '무슨 사업 해?'처럼 회사 없는 서술 질문은 summary.\n"
