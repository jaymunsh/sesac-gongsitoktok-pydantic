...
공시 발생일/질문 시점 기준으로 그 시점 거시지표를 조회해 거시 결합에 쓴다.
...
def fetch_statistic(
    stat_code: str, cycle: str, start: str, end: str,
    *, item_code1: str | None = None, rows: int = 100,
) -> list[dict]:
    """ECOS 통계 시계열 조회. cycle: D(일)/M(월)/Q(분기)/A(년)."""
...
def macro_snapshot(date: str) -> dict:
    """기준일(YYYYMMDD)의 거시 스냅샷(해당일 또는 직전 영업일 값)."""
    out: dict = {"as_of": date, "indicators": {}}
    targets = {"usd_krw": USD_KRW, "base_rate": BASE_RATE, "market_rate": MARKET_RATE, "kospi": KOSPI}
    for name, spec in targets.items():
        try:
            row = _latest_on_or_before(spec, date)
            if row:
                out["indicators"][name] = {
                    "value": row["DATA_VALUE"], "time": row["TIME"],
                    "unit": row.get("UNIT_NAME", ""), "name": row.get("ITEM_NAME1", ""),
                }
